...
class CustomImageFolder(datasets.ImageFolder):
    ...
    def _find_classes(self, dir):
        class_to_idx={"Group1":[0,0],"Group2":[1,1],"Group3":[2,1]}
...
parser.add_argument('-b', '--batch-size', default=, type=int,
                    metavar='N',
                    help='mini-batch size (default: 256), this is the total '
                         'batch size of all GPUs on the current node when '
                         'using Data Parallel or Distributed Data Parallel')
parser.add_argument('--lr', '--learning-rate', default=, type=float,
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.fine_tuning:
        ...
        if args.arch.find('densenet121') != -1:
            checkpoint = torch.load("./densenet121_model_best.pth.tar")
    ...
    criterion2 = nn.CrossEntropyLoss(weight=torch.Tensor([1, 3, 3])).cuda(args.gpu)# new
    ...
    if args.arch.find('alexnet') != -1:
        ...
        # fine_tune_parameters = [p for p in model.parameters() if p.requires_grad]
    elif args.arch.find('resnet') != -1:  # ResNet
        fine_tune_parameters = model.module.fc.parameters()
    else:
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        train(train_loader, model, criterion, criterion2, optimizer, epoch, args)

        # evaluate on validation set
        acc1,loss_2 = validate(val_loader, model, criterion, criterion2, args)
...
def validate(val_loader, model, criterion, criterion2, args):
    ...
    return top1_2.avg, losses2.avg #保存level2最好的模型
